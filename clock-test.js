...
QUnit.module( "Clock Date", () => {
  test( "initialize clock date", t => {
        t.equal( Clock.ClockDate.initializeLabel(new Date('August 19, 1975 23:15:30')), ' MARDI 19 AOUT 1975'
        , "initialize date passed" );
        
        t.equal( Clock.ClockCommonTraits.initializePositions(['A','B','c'])
                          .map((item) => item.point)
                          .toString()
  ...
  test( "calculate new position", t => {
        t.deepEqual(Clock.ClockCommonTraits.getNewPosition(
            Clock.ClockCommonTraits.initializePositions(['A','B','c']),
      ...
      t.equal(Math.round(Clock.ClockDate.xOffset(10, 0, 0, Math.PI/3 ))
          , 10
          , "xOffset at index 0")
      t.equal(Math.round(Clock.ClockDate.xOffset(10, 0, 1, Math.PI/3 ))
          , 5
          , "xOffset at index 1")
      t.equal(Math.round(Clock.ClockDate.xOffset(10, 0, 2, Math.PI/4 ))
          , 0.0
          , "xOffset at index 2")
      t.equal(Math.round(Clock.ClockDate.yOffset(10, 0, 0, Math.PI/3 ))
          , 0
          , "yOffset at index 0")
      t.equal(Math.round(Clock.ClockDate.yOffset(10, 0, 1, Math.PI/6 ))
          , 5
          , "yOffset at index 1")
      t.equal(Math.round(Clock.ClockDate.yOffset(10, 0, 2, Math.PI/4 ))
          , 10
          , "yOffset at index 2")
  });
});
